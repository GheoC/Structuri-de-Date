...
private:
	void dfsUtil(int v, bool visited[])
	{
		...
		for (int i = 0; i < noduri[v].noduriAdiancete.size(); i++)
		{
			if (!visited[noduri[v].noduriAdiancete[i]])
				dfsUtil(noduri[v].noduriAdiancete[i], visited);
		}
	}
...
private:
	void printAllPathsUtil(int u, int d, bool visited[], int path[], int& path_index)
	{
		...
		{
			for (int i = 0; i < noduri[u].noduriAdiancete.size(); i++)
			{
				if (!visited[noduri[u].noduriAdiancete[i]])
					printAllPathsUtil(noduri[u].noduriAdiancete[i], d, visited, path, path_index);
			}
